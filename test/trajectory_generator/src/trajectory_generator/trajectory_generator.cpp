  ...
  // , started(false)
{
  ...
  ROS_INFO("Initializing opetimizer...");
  optimizer = nlopt::opt(nlopt::LD_SLSQP, optimize_num * 2);
  optimizer.set_min_objective(costFunction, NULL);
  ROS_INFO("Initialized opetimizer successfully");
...
double TrajGen::costFunction(const std::vector<double> &a, std::vector<double> &grad, void* data)
{
  return 1.0;
}
